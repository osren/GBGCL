...
def run(args):
    ...
    for trial in range(trials):
        ...
        target_cc_layer = CellComplexLayer(
            in_channels_0=data.x.size(1),
            in_channels_1=hidden_dim,  # 修复：使用hidden_dim而不是1
            hidden_dim=hidden_dim
        ).to(device)
        target_model = CellComplexTarget(target_cc_layer, hidden_dim).to(device)
        ...
        with tqdm(total=num_epochs, desc='训练') as pbar:
            for epoch in range(num_epochs):
                ...
                if target_loss < best_target_loss:
                    ...
                    # 删除旧的target模型文件
                    for f in glob.glob(f'pkl/pkl_target/best_target_{tag}_*.pkl'):
                        try:
                            os.remove(f)
                        except:
                            pass
