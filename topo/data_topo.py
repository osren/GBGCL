...
def convert_graph_to_cell_complex(data, directed=False, max_cycles=10000, max_cycle_size=5):
    ...
    print(f"最大环大小: {max_cycle_size}, 最大环数量: {max_cycles}")
    ...
    # 临时移除标签相关属性
    sampled_data.y = None
    if hasattr(sampled_data, 'train_mask'):
        delattr(sampled_data, 'train_mask')
    if hasattr(sampled_data, 'val_mask'):
        delattr(sampled_data, 'val_mask')
    if hasattr(sampled_data, 'test_mask'):
        delattr(sampled_data, 'test_mask')
    ...
    try:
        print("环结构检测前处理: 确认graph-tool库版本和API...")
        import graph_tool as gt
        print(f"graph-tool版本: {gt.__version__ if hasattr(gt, '__version__') else '未知'}")
        ...
        if len(ring_complexes) > 0:
            ...
            print(f"获取复形数据，类型: {type(complex_data)}")
        else:
            print("环复形列表为空")
