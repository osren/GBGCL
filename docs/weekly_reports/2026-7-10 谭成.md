...
上周完成了 BTCM 的架构设计（见 `docs/RUNBOOK_NEXT_2026-07-02.md §四.2`），本周的目标是把"伪代码 → 代码"。当前 GCN 编码器的消息函数只用到两端节点特征，粒球信息只能通过末尾的 `granule_diffuse_and_write` 加性回写，无法在 GCN 内部"乘性"耦合。BTCM 把消息函数扩展为
...
| 文件 | 变更 |
|------|------|
| `src/models.py` | 新增 `BallConv`（in_dim×2 + ball_dim×2 → out_dim，scatter_mean + BN + PReLU）；`Conv` 加 `btcm` / `ball_dim` 开关；`Online` / `Target` 加 `gb_ball_feat` 透传 |
| `src/gb_utils.py` | 新增 `build_ball_tensor(H_ball, GB_node_list, N, device)`，构造节点→球的查表张量 |
| `src/train.py` | 加 `--gb_btcm` / `--gb_ball_emb_dim=1024` 参数；`run()` 顶层构建 `gb_ball_tensor`；`train_online` / `train_target` 共享同一张量；BTCM 通路用 `prev_H_ball`，epoch 0 走 `torch.zeros(N, ball_dim)` 占位 |
| `scripts/smoke_btcm.sh` | 50ep Photo 冒烟脚本（含 conda activate） |
| `scripts/start_smoke.sh` | nohup 启动器，写日志到 `logs/nohup/btcm_smoke4.out` |

**关键 commit**：`e52388c wip: BTCM scaffold (BallConv + build_ball_tensor + prev_H_ball pipe), ball_dim=1024 default` / `79f492e fix:修复TRAIN.PY` / `5a362d4 fix:继续修复`。
...
- 每组 700ep × 5trial，对照 `docs/PROGRESS_2026-07-02.md` Stage-B Top-1 表
...
|------|--------|------|---------------|---------|---------------|
| R-A | Photo | Phase 0 baseline（SGRL 等价，无 GB） | **0.9391 ± 0.0007** | -0.04pp | — |
...
1. 之前 "Photo -0.20pp vs SGRL" 的 finding **不准确** —— Phase 0 baseline 自身就 -0.04pp，**SGRL 的 93.95% 本身就是上限附近**，并非我们的粒球模块把 SGRL 拉下来了
...
Phase R Photo / Computers 完成，已写入 `results/phaseR/<dataset>_summary.csv`，本地用 `tools/analyze_phaseR.py` 出表。CS / Physics 排队等 GPU 让出（BTCM 50ep 占 1 张 4090）。
...
|------|-------------|-----------------|------------|
| common.sh `--data_dir` 绝对路径 | `5ba194f` | scp `common.sh` + 重启 Phase W | 否（本机防火墙挡 github） |
| data.py 下载 monkey-patch | (在服务器 sed 注入) | 服务器侧 sed 写入 | 否 |
| BTCM scaffold | `e52388c` / `79f492e` / `5a362d4` | scp `models.py` `gb_utils.py` `train.py` 全量替换 | 否 |
...
- **消融实验设计修订**：Phase R 的新结论（GBGCL 对 Photo / Computers ≈ SGRL ± 噪声）让消融章节的写法调整——Photo 不再当作"亮点"，CS 94.28% + BTCM smoke 0.8897 / 50ep 才是需要重点放大的点。消融维度同步改为"GB 注入位置（conv1 / conv2 / both）× ball_emb_dim（64 / 256 / 1024）× quity（detach / homo）× 球损失权重"四维网格，把 BTCM 与 Phase R 的实验数据合并到一张大表里。
- **结果对比表草稿**：4 数据集 × {baseline, GBGCL-Stage-B-Top-1, GBGCL-BTCM} × 5trial 的对比框架已搭好，等待 BTCM 完整 700ep × 5trial 数据回填。Photo / Computers 现阶段结论"与 SGRL 持平"先占位写"GBGCL 不退化"，CS / Physics 留空待 BTCM 跑完补。
...
第二阶段是 BTCM 完整实验（7/13–7/16），把 Photo / Computers / CS / Physics 四数据集 × 700ep × 5trial 的 BTCM 跑齐，重点关注 BTCM 是否能在 Phase R baseline 基础上显著拉高（目标 Δ ≥ +0.3pp / 数据集才算 BTCM 真正起效）；同时把 `ball_diffusion_btcm()` 真正嵌入 InfoNCE 的核心算法实现出来——当前 BTCM 只用 ball_feat 做消息注入，没有把球扩散嵌入 InfoNCE（runbook §四.2 设计的目标形态），下半周要把"球邻居对作为 InfoNCE 正样本"这一关键改动落地并跑 smoke 验证。
